...
public class SimpleMathResource {
	...
	public SimpleMathResource() {
	     ...
	     ConfigurationManager.getConfigInstance().setProperty("hystrix.command.DateHystrixCommand.execution.isolation.thread.timeoutInMilliseconds", 3000);
    ...
    public Response sum(@PathParam("v1") Double v1,
        ...
        try {
        	...
        	HystrixCommandMetrics dateMetrics = HystrixCommandMetrics.getInstance(HystrixCommandKey.Factory.asKey("DateHystrixCommand"));
        	
        	JSONObject response = new JSONObject();
            response.put("Message", "Sum [" + v1 + "] + [" + v2 + "] + = " + (v1+v2) + " | from Netflix OSS - Date: " + d.toString() + " - " + dateMetrics);
    ...
    public String sumRx(Double v1,Double v2) {
        ...
        try {
        	...
        	JSONObject response = new JSONObject();
        	String result = "Sum [" + v1 + "] + [" + v2 + "] + = " + (v1+v2) + " | from Netflix OSS - Date: " + d.toString() + " - " + dateMetrics;
            response.put("Message",result);
            
            logger.info("Math Response : " + result);
